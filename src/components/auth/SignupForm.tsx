...
export default function SignupForm() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    if (error) toast({ title: t("common.error"), description: String(error.message), variant: "destructive" });
  };
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button type="button" onClick={() => setRole("parent")} className={`flex items-center justify-center gap-2 p-3 rounded-lg border-2 transition-all ${role === "parent" ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground hover:border-primary/40"}`}>
                <Users className="h-5 w-5" /><span className="font-semibold">{t("common.parent")}</span>
              </button>
              <button type="button" onClick={() => setRole("child")} className={`flex items-center justify-center gap-2 p-3 rounded-lg border-2 transition-all ${role === "child" ? "border-secondary bg-secondary/10 text-secondary" : "border-border text-muted-foreground hover:border-secondary/40"}`}>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <Input id="signup-password" type="password" placeholder={t("auth.passwordPlaceholder")} value={password} onChange={(e) => setPassword(e.target.value)} className="pl-10" required minLength={6} />
            </div>
          </div>
        </CardContent>
